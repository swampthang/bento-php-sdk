...
final class BentoCommandsTest extends TestCase
{
    ...
    public function testCanChangeEmail()
    {
        ...
        $result = $bento->V1->Commands->changeEmail([
            'old_email' => 'test@bentonow.com',
            'new_email' => 'test2@bentonow.com',
        ]);

        $requests = Server::received();

        $this->assertEquals(
            '{"command":{"command":"change_email","email":"test@bentonow.com","query":"test2@bentonow.com"},"site_uuid":"test"}',
            $requests[0]->getBody()->getContents()
        );
