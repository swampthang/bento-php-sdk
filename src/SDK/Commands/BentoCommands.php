...
class BentoCommands
{
    ...
    private function _constructCommandsResult($response)
    {
        ...
        if ($this->_logger) {
            $this->_logger->info('Decoded Bento response:', ['response' => $decodedResponse]);  // Log here
        }
     ...
     * @throws \Exception
     */
    public function changeEmail($parameters, $logger)
    {
        ...
        $this->_logger->info('Decoded Bento response:', ['response' => $response]);
